...
int V_snprintf( char *pDest, int bufferLen, char const *pFormat, ... )
{
	Assert( bufferLen > 0 );
	Assert( pDest != NULL );
	Assert( pFormat != NULL );
...
bool V_RemoveDotSlashes( char *pFilename, char separator )
{
	...
	// Get rid of a trailing "/." (needless), but leave the path separator.
...
{
	return strchr( pStr, ':' ) != NULL || pStr[0] == '/' || pStr[0] == '\\';
}
...
static DWORD ConvertWideToUTF8( const wchar_t *pwszNullTerm, char *pchOut, DWORD nMaxOut )
{
	...
	DWORD ret = WideCharToMultiByte( CP_UTF8, 0, pwszNullTerm, -1, pchOut, nMaxOut, NULL, NULL );
	if ( ret == 0 )
	{
		// XXX is it better to generate incomplete UTF-8 or to drop multiple bytes?
		// we'll go with incomplete UTF-8 since the return value is bogus anyway.
		pchOut[ nMaxOut - 1 ] = 0;
	}
	return ret;
}

char *getcwd_utf8( char *pszUtf8PathOut, size_t size )
{
	// explicitly defined to return a malloc'd buffer
	wchar_t *pwszMallocTemp = _wgetcwd( NULL, 0 );
	if ( !pwszMallocTemp )
		return NULL;

	DWORD nBytesRequired = ConvertWideToUTF8( pwszMallocTemp, NULL, 0 );
	if ( !pszUtf8PathOut )
	{
		if ( size < nBytesRequired )
			size = nBytesRequired;

		// explicitly defined to return a CRT malloc'd buffer
		pszUtf8PathOut = (char*) malloc( size );
	}

	ConvertWideToUTF8( pwszMallocTemp, pszUtf8PathOut, (DWORD)MIN( size, (size_t)INT_MAX ) );
	free( pwszMallocTemp );

	if ( size < nBytesRequired )
	{
		// won't get hit in our malloc case, we made sure size >= nBytesRequired
		errno = ERANGE;
		return NULL;
	}

	return pszUtf8PathOut;
}

#elif defined(OSX) || defined(LINUX) || defined(POSIX)
...
void V_MakeAbsolutePath( char *pOut, int outLen, const char *pPath, const char *pStartingDir )
{
	...
	else
	{
		...
		{
			if ( !getcwd_utf8( pOut, outLen ) )
			{
				AssertMsg( false, "V_MakeAbsolutePath: _getcwd failed." );
				if ( outLen )
					*pOut = 0;
			}
	...
	// this will also fix slashes
	if ( !V_RemoveDotSlashes( pOut ) )
		AssertMsg1( false, "V_RemoveDotSlashes failed for \"%s\".", pOut );
}
