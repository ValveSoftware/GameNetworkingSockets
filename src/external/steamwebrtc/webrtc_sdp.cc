...
// NOTE: This file is a modified ("violently hacked" might be more
// appropriate term) copy the file of the same name in the webrtc code.
//
...
// one unfortunate exception.  We would like to use the standard
// text format to describe ICE candidates.  Unfortunately, that code
// is intermixed with the other code that does all the rest of the SDP
// parsing, including media formats, etc.  Hence the hacks.  Actually,
// even this file is well layered, as evidenced by the fact that the only
// changes we need to make are to comment out the higher layers that we do
// not need.  If function-level linking is used, no hacks are necessary!
// (Or if the WebRTC authors would split up this file, no hacks would be
// necessary -- we don't have the clout to ask for that.)  But if the typical
// file-level linking is used, the higher levels are brought in, which
// brings in an avalanche of dependencies.  An attempt was made to butcher
// this file with as few textual changes as possible; since the changes amount
// to only commenting out blocks of code, that was straightforward.  No
// "changes" were actually necessary.  You can see what was commented
// out by searching for for STEAMNETWORKINGSOCKETSHACK.
